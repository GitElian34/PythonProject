"""
plot_5stations_27j_divergence_modele_bon_alti_mauvais.py
════════════════════════════════════════════════════════════════════════
Variante de generalized_top5_zscore_vs_metres.py : au lieu de prendre les
5 MEILLEURES stations (score = min(NSE alti-insitu, NSE modèle-insitu)),
on sélectionne 5 stations "moyennes" où les deux métriques DIVERGENT :

    NSE(modèle, insitu) BON   ET   NSE(alti, insitu) MAUVAIS

Intérêt : ce sont les cas les plus informatifs pour comprendre le
problème encore ouvert (écart1, alti vs insitu) — si le modèle s'accorde
bien avec l'insitu alors que l'alti brute ne s'accorde pas avec l'insitu,
ça isole visuellement où et comment l'alti et l'insitu divergent, sans
que le modèle soit en cause (puisque lui, justement, s'accorde bien).

Critère de sélection : on classe les stations 27j par
    gap = NSE(modèle, insitu) - NSE(alti, insitu)
et on garde les 5 plus grands gaps, sous réserve que NSE(modèle, insitu)
reste correct (seuil MIN_NSE_MODELE_BON) -- sinon un grand gap pourrait
juste vouloir dire "les deux sont mauvais mais le modèle un peu moins".

Mêmes 2 panels que le script précédent (z-score / mètres avec insitu
recalée), mêmes conventions (formule de reconstruction validée,
normalize_code, KGE sans biais, SWORD chargé une seule fois).

Usage :
    python plot_5stations_27j_divergence_modele_bon_alti_mauvais.py
════════════════════════════════════════════════════════════════════════
"""
...
SOURCE = "dahiti"
MASKS = [80, 90, 96]
FREQ = "27j"           # uniquement 27j pour cette analyse
N_SELECT = 5

MIN_NSE_MODELE_BON = 0.7   # seuil pour considérer "modèle vs insitu bon"
# pas de seuil dur côté "alti mauvais" -> on prend simplement le plus grand
# gap parmi les stations qui passent le seuil ci-dessus, le mauvais score
# alti ressort naturellement.
...
OUTPUT_BASE = Path("./data_processing/AnalyseModelsDtoD/plot/meter_divergence_27j")
OUTPUT_BASE.mkdir(parents=True, exist_ok=True)

DATE_MIN, DATE_MAX = "2016-01-01", "2025-12-31"
DIST_MAX_KM = 50.0
FACC_MAX_RATIO = 2.0
WINDOW_ALTI_INSITU = 14   # pour 27j
...
# ═══════════════════════════════════════════════════════════════
# 2. RÉSIDUS COMPLETS — 27j x 3 MASQUES
# ═══════════════════════════════════════════════════════════════
print("### Chargement des résidus complets (27j x 80/90/96) ###")
FULL_RESIDUALS = {}
stations_27j = set()
...
for mask in MASKS:
    ...
    FULL_RESIDUALS[mask] = df
    stations_27j |= set(normalize_code(c) for c in df["station"].unique())
...
# ═══════════════════════════════════════════════════════════════
# 4. SÉLECTION PAR DIVERGENCE (modèle bon, alti mauvais)
# ═══════════════════════════════════════════════════════════════
print(f"### Sélection par divergence — {FREQ} ###")
rows = []
for _, sta in df_stations.iterrows():
    code = sta["station_code"]
    code_n = normalize_code(code)
    if code_n not in stations_27j:
        continue
    ...
    ins_wl_a = align_insitu(df_alti["date"].values, df_ins, WINDOW_ALTI_INSITU)
    ...
    rows.append({
        ...
        "gap_modele_moins_alti": round(best_nse_model - nse_alti_ins, 3),
    })

if not rows:
    raise SystemExit("⚠ Aucune station valide -> revoir les filtres.")

df_scores = pd.DataFrame(rows)
df_bon_modele = df_scores[df_scores["nse_modele_insitu"] >= MIN_NSE_MODELE_BON].copy()
if df_bon_modele.empty:
    print(f"  ⚠ Aucune station avec NSE(modèle,insitu) >= {MIN_NSE_MODELE_BON} -> "
          f"on relâche le seuil et on prend les plus gros gaps toutes stations confondues.")
    df_bon_modele = df_scores.copy()

df_select = df_bon_modele.sort_values("gap_modele_moins_alti", ascending=False).head(N_SELECT)
out_csv = OUTPUT_BASE / f"selection_divergence_{FREQ}.csv"
df_select.to_csv(out_csv, index=False)
print(f"\n{len(df_scores)} stations évaluées, {len(df_bon_modele)} avec NSE modèle >= {MIN_NSE_MODELE_BON}")
print(f"TOP {N_SELECT} par divergence (modèle bon, alti mauvais) -> {out_csv}")
print(df_select.to_string(index=False))

# ═══════════════════════════════════════════════════════════════
# 5. PLOTS — pour chaque station sélectionnée, chaque année
# ═══════════════════════════════════════════════════════════════
def plot_station(code, code_ins, mask, dist_km):
    code_n = normalize_code(code)
    df_alti = get_alti_series(code)
    df_ins = get_insitu_series(code_ins)
    sub = FULL_RESIDUALS[mask]
    ...
    ins_wl_a_full = align_insitu(df_alti["date"].values, df_ins, WINDOW_ALTI_INSITU)
    ...
    pred_z = sub["pred"].values  # déjà z-scoré en amont
    ...
    for year in all_years:
        ...
        ax_z.set_title("Alti vs Insitu", fontsize=10, fontweight="bold")
        ...
        fig.suptitle(
            f"DAHITI {code} ({FREQ}) — Insitu SWORD {code_ins} ({dist_km} km) — DtoD{mask}% — {year}\n"
            f"{ligne_metriques}",
    ...
    print(f"  Station {code} (insitu {code_ins}, {dist_km} km, DtoD{mask}%) : {n_figs} figures -> {out_dir}")


print(f"\n### Génération des figures ###")
for _, row in df_select.iterrows():
    plot_station(row["station"], row["insitu"], row["best_mask"], row["dist_km"])
