"""
plot_outliers_with_insitu.py
═══════════════════════════════════════════════════════════════════════════
Identique à plot_outliers.py mais ajoute un panneau insitu en dessous :
  [1] Obs satellite vs Prédit modèle — outliers annotés
  [2] Obs satellite vs Station insitu la plus proche (z-score)

Usage :
  python plot_outliers_with_insitu.py
═══════════════════════════════════════════════════════════════════════════
"""
...
BASE_DIR = Path(f"./figures_zeroshot_satellite/{MODEL}/Outlier_insitu")
OUTPUT_DIR= Path("./AI/LSTM/NeuralHydro/Comparaison_insitu/station_plot")
...
def get_coords_hydro(station_code):
    ...
    df   = pd.read_sql_query(
        ...
        conn, params=(str(station_code).zfill(13),)
    )
    conn.close()
    if df.empty:
        return None, None
...
# ═══════════════════════════════════════════════════════════════
# On pré-calcule le matching insitu une fois par station
_cache = {}

def get_insitu_info(sta, date_min, date_max):
    """Retourne (code_insitu, dist_km, df_insitu) avec cache."""
...
for sta in stations:
    ...
    code_insitu, dist_km, df_ins = get_insitu_info(
        sta, grp['date'].min(), grp['date'].max()
    )
    ...
    for year in years_with_outliers:
        ...
        # Données insitu alignées sur l'année
        insitu_wl_norm = None
        ...
        n_out = len(out_year)
        ax_wl.set_title(
            f"Station {sta}  —  {year}  —  {n_out} outlier{'s' if n_out > 1 else ''}",
