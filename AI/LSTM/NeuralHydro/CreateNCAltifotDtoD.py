"""
create_dataset_hwnext_DtoD.py
════════════════════════════════════════════════════════════════════════
Crée les .nc HW Next au format compatible avec les modèles DtoD
(données quotidiennes, features J0/J3/J10, water_level aux vraies
dates de mesure satellite et NaN ailleurs).

Utilisé pour l'évaluation zero-shot des 5 modèles AR-LSTM
entraînés avec différents taux de masquage.

Sorties :
  ./data/IA/NeuralHydrologyHWNextDtoD/time_series/<id>.nc
  ./data/IA/NeuralHydrologyHWNextDtoD/attributes/attributes.csv
  ./data/IA/NeuralHydrologyHWNextDtoD/stations_hwnext_27j.txt
════════════════════════════════════════════════════════════════════════
"""
...
# PARAMÈTRES
# ═══════════════════════════════════════════════════════════════
DB_PATH    = "./data/hydroweb_next.db"
OUTPUT_DIR = "./data/IA/NeuralHydrologyHWNextDtoD"
DATE_DEB   = "2016-01-01"
DATE_FIN   = "2025-12-31"
CLIM_WINDOW = 20
...
print(f"{len(stations)} stations HW Next chargées")
...
for i, row in stations.iterrows():
    ...
    nc_path      = os.path.join(ts_dir, f"{station_code}.nc")
    ...
    try:
        ...
        # Place chaque mesure sur le jour exact de la grille quotidienne
        ...
        # water_level — NaN sauf aux vraies dates satellite
        ...
        if (i + 1) % 50 == 0 or i < 5:
            print(f"  [{i+1:3d}/{len(stations)}] {name:40s} → {station_code}.nc | {n_wse} WSE")
...
# ── stations txt ───────────────────────────────────────────────
txt_path = os.path.join(OUTPUT_DIR, "stations_hwnext_27j.txt")
