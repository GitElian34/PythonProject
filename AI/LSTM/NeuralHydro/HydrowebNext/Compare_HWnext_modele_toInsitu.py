"""
benchmark_hwnext_final.py
════════════════════════════════════════════════════════════════════════
Benchmark final : HW Next vs insitu & Modèle AR-LSTM vs insitu — 27j

Sources :
  - HW Next obs  : CSV résidus recalés (date_recalee + obs)
  - Modèle pred  : CSV résidus recalés (date_recalee + pred)
  - Insitu       : insitu_data.db (h_med_wsh)
  - Coords       : hydroweb_next.db

Sorties :
  - benchmark_hwnext_27j.csv
  - benchmark_hwnext_global.png  (boxplots comparatifs)
════════════════════════════════════════════════════════════════════════
"""
...
# PARAMÈTRES
# ═══════════════════════════════════════════════════════════════
CSV_27J    = "./AI/LSTM/NeuralHydro/HydrowebNext/Nexdtaset_dates_hwnext_27j.csv"
...
OUTPUT_DIR = Path("./data/outlier_detection/benchmark_hwnext")
...
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*65}")
print(f"  TRAITEMENT HW Next 27j")
print(f"{'='*65}")

df = pd.read_csv(CSV_27J)
...
for code in df["station"].unique():
    ...
    ins    = align_insitu(sub["date_recalee"].values, df_ins, WINDOW_27J)
...
csv_path = OUTPUT_DIR / "benchmark_hwnext_27j.csv"
df_out.to_csv(csv_path, index=False)

print(f"\n  Stations traitées : {len(df_out)}")
if len(df_out) > 0:
    meilleur = (df_out["nse_modele_ins"] > df_out["nse_hwnext_ins"]).sum()
    print(f"\n  {'':30} {'médiane':>8} {'moyenne':>8} {'> 0':>6} {'> 0.5':>6}")
    print(f"  {'-'*60}")
    for col, label in [
        ("nse_hwnext_ins", "HW Next obs  vs insitu"),
        ("nse_modele_ins", "Modele pred  vs insitu"),
        ("nse_modele_hw",  "Modele pred  vs HW Next"),
    ]:
        v = df_out[col].dropna()
        print(f"  {label:<30} {v.median():>8.3f} {v.mean():>8.3f} {(v>0).sum():>6} {(v>0.5).sum():>6}")
    print(f"\n  Modele meilleur que HW Next : {meilleur}/{len(df_out)} ({meilleur/len(df_out):.0%})")
...
if len(df_out) > 0:
    ...
    fig.suptitle(
        "Benchmark NSE — HW Next & Modèle AR-LSTM vs Insitu (27j)\n"
        "(dates recalées, z-scores indépendants)",
        fontsize=12, fontweight="bold"
    )

    _plot_boxplot(axes[0], {
        "HW Next\nvs insitu" : (df_out["nse_hwnext_ins"].dropna().values, C_HW),
        "Modèle\nvs insitu"  : (df_out["nse_modele_ins"].dropna().values, C_MOD),
    }, rng, title=f"HW Next & Modèle vs Insitu  (n={len(df_out)})")

    _plot_boxplot(axes[1], {
        "Modèle\nvs HW Next" : (df_out["nse_modele_hw"].dropna().values, C_MOD),
    }, rng, title=f"Modèle vs HW Next  (n={len(df_out)})")

    plt.tight_layout()
    fig_path = OUTPUT_DIR / "benchmark_hwnext_global.png"
