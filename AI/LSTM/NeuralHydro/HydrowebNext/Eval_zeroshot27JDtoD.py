"""
eval_zeroshot_hwnext_27j_DtoD.py
════════════════════════════════════════════════════════════════════════
Évaluation zero-shot des modèles AR-LSTM DtoD sur les stations
HW Next à fréquence ~27j.

Étapes :
  1. Filtrer les .nc HW Next avec gap médian entre 22 et 32j
...
Sorties par modèle :
  residuals_hwnext_27j_{mask}pct.csv  → obs, pred, date, résidus
  results_per_station_{mask}pct.csv   → NSE/KGE par station
  benchmark_DtoD_hwnext27j.csv        → tableau comparatif global

Usage :
    python eval_zeroshot_hwnext_27j_DtoD.py
════════════════════════════════════════════════════════════════════════
"""
...
STATIONS_OUT = Path("./data/IA/NeuralHydrologyHWNextDtoD/stations_hwnext_27j_eval.txt")
OUTPUT_DIR   = Path("./data/outlier_detection/benchmark_DtoD_hwnext27j")
RUNS_DIR     = Path("./runs")

GAP_MIN = 22    # filtre serré autour de 27j
GAP_MAX = 32
...
# ═══════════════════════════════════════════════════════════════
# ÉTAPE 1 — FILTRE STATIONS ~27j
# ═══════════════════════════════════════════════════════════════
print("=" * 60)
print("  ÉTAPE 1 — Filtre stations HW Next ~27j")
...
for f in sorted(NC_DIR.glob("*.nc")):
    ...
    if GAP_MIN <= gap_med <= GAP_MAX:
        stations_27j.append(f.stem)

STATIONS_OUT.write_text("\n".join(stations_27j))
print(f"  Stations retenues : {len(stations_27j)}")
print(f"  Fichier stations  : {STATIONS_OUT}\n")

if len(stations_27j) == 0:
...
for model_name, cfg_info in MODELS.items():
    ...
    config_eval = run_dir / "config_eval_hwnext_27j.yml"
    ...
    for station, data in raw.items():
        if str(station) not in stations_27j:
            continue
        try:
            freq_key = list(data.keys())[0]
    ...
    # ── Résidus ───────────────────────────────────────────────
    df_res = extract_residuals(raw, stations_27j)
    res_csv = OUTPUT_DIR / f"residuals_hwnext_27j_{mask}pct.csv"
...
if not results_summary:
    ...
    csv_path = OUTPUT_DIR / "benchmark_DtoD_hwnext27j.csv"
    df_cmp.to_csv(csv_path, index=False)

    print(f"\n{'='*65}")
    print("  RESULTATS COMPARATIFS — Zero-shot HW Next ~27j")
    print(f"{'='*65}")
    ...
    print(f"    benchmark_DtoD_hwnext27j.csv")
    print(f"    results_per_station_{{mask}}pct.csv")
    print(f"    residuals_hwnext_27j_{{mask}}pct.csv")
    print(f"\nDone")