...
BASINS_DIR      = Path("./AI/LSTM/NeuralHydro_feat10j/")
...
def permuter_variable_nc(ts_dir, test_ids, var_name, seed):
    """
    Permute les valeurs d'une variable dans le temps pour chaque .nc.
    ...
    for sid in test_ids:
        ...
        valid_vals = vals[mask_valid]
        rng.shuffle(valid_vals)
        vals[mask_valid] = valid_vals
...
if __name__ == "__main__":
    print("=" * 60)
    print("PERMUTATION IMPORTANCE — VARIABLES DYNAMIQUES")
    print(f"Run : {RUN_DIR}")
    print(f"Epoch : {EPOCH}")
    print("=" * 60)
    ...
    try:
        ...
        for i, var in enumerate(DYNAMIC_VARS):
            ...
            # Permuter dans le dossier temp
            permuter_variable_nc(ts_temp, test_ids, var, seed=SEED + i)
