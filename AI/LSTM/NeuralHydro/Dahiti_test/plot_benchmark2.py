...
Règles clés :
  ...
  - L'insitu est aligné sur les MÊMES dates pour les 3 panels
...
RESIDUALS_27J = "./data/outlier_detection/residuals_27j_dahiti.csv"
RESIDUALS_10J = "./data/outlier_detection/residuals_10j_dahiti.csv"
STATIONS_27J  = "./data/IA/NeuralHydrologyDahitiFull/stations_dahiti_27j.txt"
STATIONS_10J  = "./data/IA/NeuralHydrologyDahitiFull/stations_dahiti_10j.txt"

OUTPUT_DIR         = Path("./data/outlier_detection/plots_dahiti")
DATE_MIN, DATE_MAX = "2016-01-01", "2025-12-31"
DIST_MAX_INSITU_KM = 50.0

# Couleurs
C_DA  = "#27ae60"   # vert DAHITI
...
def get_coords(conn, code):
    """Cherche les coords dans dahiti.db — dahiti.db stocke en zfill(13)."""
...
def run_freq(stations_file, residuals_csv, freq_label, window_insitu):
    ...
    for i, code in enumerate(stations):
        # Cherche les coords avec le code tel quel (DAHITI n'a pas de zfill 13)
        ...
        # Résidus modèle — le station_id dans le CSV peut avoir différents formats
        ...
        # ── Grille unifiée : dates DAHITI avec obs & pred OK ──
        ...
        # z-scores
        obs_z      = zscore(sub["obs"].values)    # DAHITI normalisé
    ...
    if len(df_out) == 0:
        print("  ⚠ Aucune station traitée — vérifier get_coords / matching CSV")
    else:
        for col, label in [
...
fig.suptitle(
    ...
    "(grille temporelle unifiée : dates DAHITI avec obs & pred valides)",
