"""
step4_create_dataset_dahiti_27j_clean.py
════════════════════════════════════════════════════════════════════════
Recrée les .nc DAHITI 27j avec UNE SEULE mesure par passage satellite
(élimine les doublons S3A/S3B à 5j d'écart).

Différence avec step4_create_dataset_dahiti.py :
  - Filtre les mesures DAHITI pour ne garder qu'une mesure toutes les
    ~27j (seuil minimum 20j entre deux mesures consécutives)
  - Prend la première mesure du passage (S3A en général)
  - Sorties dans ./data/IA/NeuralHydrologyDahiti27jClean/

Sorties :
  - ./data/IA/NeuralHydrologyDahiti27jClean/27j/time_series/<id>.nc
  - ./data/IA/NeuralHydrologyDahiti27jClean/27j/attributes/attributes.csv
  - ./data/IA/NeuralHydrologyDahiti27jClean/stations_dahiti_27j.txt
════════════════════════════════════════════════════════════════════════
"""
...
log = logging.getLogger("step4_dahiti_27j_clean")

# ═══════════════════════════════════════════════════════════════
# PARAMÈTRES
# ═══════════════════════════════════════════════════════════════
DB_PATH     = "./data/dahiti.db"
OUTPUT_DIR  = "./data/IA/NeuralHydrologyDahiti27jClean"
DATE_DEB    = "2016-01-01"
DATE_FIN    = "2025-12-31"
CLIM_WINDOW = 20
MIN_GAP_DAYS = 20   # écart minimum entre deux mesures consécutives
...
# ═══════════════════════════════════════════════════════════════
# FILTRE 27j — UNE MESURE PAR PASSAGE
# ═══════════════════════════════════════════════════════════════
def filter_one_per_pass(df_wse: pd.DataFrame, min_gap: int = MIN_GAP_DAYS) -> pd.DataFrame:
    """
    Garde une seule mesure toutes les ~27j.
    Élimine les doublons S3A/S3B (qui sont à 5j d'écart).
...
def load_stations(conn):
    return pd.read_sql("""
        ...
        WHERE s.reference_longitude IS NOT NULL
          ...
          AND (s.mission_track LIKE '%S3A%' OR s.mission_track LIKE '%S3B%')
...
def load_wse_station_filtered(conn, station_code, date_range):
    ...
    # FIX : filtre une mesure par passage (~27j)
    df_filtered = filter_one_per_pass(df)

    log.info(f"    WSE : {len(df)} mesures BDD → {len(df_filtered)} après filtre 27j")
...
def run():
    ...
    ts_dir   = os.path.join(OUTPUT_DIR, "27j", "time_series")
    att_dir  = os.path.join(OUTPUT_DIR, "27j", "attributes")
    os.makedirs(ts_dir,  exist_ok=True)
    os.makedirs(att_dir, exist_ok=True)

    stations = load_stations(conn)
    log.info(f"{len(stations)} stations S3A/S3B chargées")
    ...
    # stations txt
    txt_path = os.path.join(OUTPUT_DIR, "stations_dahiti_27j.txt")
    ...
    print(f"  Dossier      : {OUTPUT_DIR}/27j/")
    print("═" * 60)
