"""
analyse_zeroshot_facteurs_27j.py
═══════════════════════════════════════════════════════════════════════════
Analyse croisée : qu'est-ce qui fait qu'une station satellite 27j est bien
prédite en zero-shot vs mal prédite ?

Lit les métriques directement depuis validation_metrics.csv.

Sortie :
  ./Exploring_data/Analyse_zeroshot_27j/
═══════════════════════════════════════════════════════════════════════════
"""
...
NC_DIR      = './data/IA/NeuralHydrology_satellite_27D/time_series/'
ATTR_CSV    = './data/IA/NeuralHydrology_satellite_27D/attributes/attributes.csv'
OUT_DIR     = Path('./Exploring_data/Analyse_zeroshot_27j')
...
# ═══════════════════════════════════════════════════════════════
df_met = pd.read_csv(METRICS_CSV, header=None, names=["station", "NSE", "KGE"])
df_met["NSE"] = pd.to_numeric(df_met["NSE"], errors="coerce")
df_met["KGE"] = pd.to_numeric(df_met["KGE"], errors="coerce")
df_met["station"] = df_met["station"].astype(str)
METRICS = dict(zip(df_met["station"], df_met["NSE"]))

conn = sqlite3.connect(DB_PATH)

print("╔══════════════════════════════════════════════════════════════╗")
print(f"║  ANALYSE DES FACTEURS DE SUCCÈS — ZERO-SHOT {FREQ_LABEL:>4s}            ║")
print(f"║  {len(METRICS)} stations                                              ║")
print("╚══════════════════════════════════════════════════════════════╝\n")
...
for code_sta, nse in METRICS.items():
    ...
    if not df_mes.empty:
        ...
        if len(intervals) > 0:
            ...
            row['nb_gaps_90j'] = np.sum(intervals > 90)
    ...
    if os.path.exists(nc_path):
        ...
        if is_nan.any():
            ...
            if count > 0:
                nan_runs.append(count)
            row[f'max_trou_consecutif_{FREQ_JOURS}D'] = max(nan_runs) if nan_runs else 0
            row['nb_trous'] = len(nan_runs)
        else:
            row[f'max_trou_consecutif_{FREQ_JOURS}D'] = 0
            row['nb_trous'] = 0

        for var in ['precipitation_J0', 'precip_mean_J27', 'temperature_J0']:
            if var in ds:
                vals = ds[var].values
...
print(f"\n  {'Variable':<35} {'Spearman ρ':>11} {'p-value':>9} {'Pearson r':>10} {'Verdict'}")
print("  " + "─" * 85)
for _, row in df_corr.iterrows():
    sig = "***" if row['spearman_p'] < 0.001 else "**" if row['spearman_p'] < 0.01 else "*" if row['spearman_p'] < 0.05 else ""
    verdict = ""
    if abs(row['spearman_rho']) > 0.3 and row['spearman_p'] < 0.05:
        direction = "↑" if row['spearman_rho'] > 0 else "↓"
        verdict = f"IMPORTANT {direction}"
    print(f"  {row['variable']:<35} {row['spearman_rho']:>+10.3f} {row['spearman_p']:>9.4f} "
...
print(f"\n  {'Variable':<35} {'Bonnes':>10} {'Mauvaises':>10} {'Diff%':>8} {'p-value':>9}")
print("  " + "─" * 77)
...
for var in compare_vars:
    ...
    print(f"  {var:<35} {med_b:>10.2f} {med_m:>10.2f} {diff_pct:>+7.0f}% {p:>8.4f} {sig}")
...
# ── Figure 1 : scatter plots top corrélations ──────────────────────────
...
axes = axes.flatten()
fig.suptitle(f"Corrélations avec le NSE zero-shot — {FREQ_LABEL} ({len(df)} stations)",
             fontsize=14, fontweight='bold')

for idx, var in enumerate(top_vars):
    ax = axes[idx]
    valid = df[['NSE', var]].dropna()
    ...
    ax.scatter(valid[var], valid['NSE'], c=colors, s=30, alpha=0.6, edgecolors='white')
...
# ── Figure 2 : barplot corrélations ────────────────────────────────────
fig, ax = plt.subplots(figsize=(12, 7))
df_corr_plot = df_corr.head(20).sort_values('spearman_rho')
...
ax.set_title(f"Top 20 corrélations — Zero-shot {FREQ_LABEL} ({len(df)} stations)")
...
# ── Figure 3 : boxplots bonnes vs mauvaises ────────────────────────────
key_vars = [v for v in ['couverture_pct', 'nb_mesures_snap', 'elevation_mean',
            'aire_km2', 'strahler', 'wl_range', 'wl_seasonal_range',
            'interval_max', 'nb_gaps_60j', f'max_trou_consecutif_{FREQ_JOURS}D',
            'slope_mean', 'duree_ans'] if v in df.columns]

n_kv = len(key_vars)
fig, axes = plt.subplots(2, (n_kv + 1) // 2, figsize=(18, 8))
axes = axes.flatten()
fig.suptitle(f"Bonnes (NSE≥{seuil:.2f}) vs Mauvaises — {FREQ_LABEL}", fontsize=14, fontweight='bold')

for idx, var in enumerate(key_vars):
    ax = axes[idx]
    data = [bonnes[var].dropna(), mauvaises[var].dropna()]
    bp = ax.boxplot(data, labels=['Bonnes', 'Mauvaises'], patch_artist=True, widths=0.5)
...
# ── Figure 4 : carte géographique ──────────────────────────────────────
if 'lon' in df.columns and 'lat' in df.columns:
    fig, ax = plt.subplots(figsize=(10, 8))
    scatter = ax.scatter(df['lon'], df['lat'], c=df['NSE'], cmap='RdYlGn',
                         s=60, edgecolors='black', linewidth=0.5, vmin=-0.3, vmax=0.9)
    plt.colorbar(scatter, ax=ax, label='NSE zero-shot')
    ax.set_xlabel("Longitude")
    ax.set_ylabel("Latitude")
    ax.set_title(f"NSE zero-shot par station satellite {FREQ_LABEL} ({len(df)} stations)")
...
# 5. EXPORT
# ═══════════════════════════════════════════════════════════════
csv_path = OUT_DIR / "tableau_complet_zeroshot_27j.csv"
...
if not sig_vars.empty:
    ...
    for _, row in sig_vars.iterrows():
        ...
        print(f"    {row['variable']:<35} ρ={row['spearman_rho']:+.3f}  → {direction}")
else:
    print("\n  Aucune corrélation fortement significative trouvée (|ρ|>0.3, p<0.05)")
    print("  Top 5 tendances :")
    for _, row in df_corr.head(5).iterrows():
        direction = "+" if row['spearman_rho'] > 0 else "-"
        print(f"    {row['variable']:<35} ρ={row['spearman_rho']:+.3f} (p={row['spearman_p']:.3f}) {direction}")

print(f"\nNSE médian : {df['NSE'].median():.3f}")
print(f"NSE > 0.5 : {(df['NSE'] > 0.5).sum()}/{len(df)}")
print(f"NSE > 0.0 : {(df['NSE'] > 0.0).sum()}/{len(df)}")
print(f"NSE < 0.0 : {(df['NSE'] < 0.0).sum()}/{len(df)}")
