"""
create_dataset_satellite_10D_v2.py
═══════════════════════════════════════════════════════════════════════════
Dataset satellite sur grille 10D RÉGULIÈRE — VERSION 2 avec SNOW + DOY

Variables dynamiques (alignées sur le meilleur config insitu) :
...
Sortie :
  ./data/IA/NeuralHydrology_satellite_10D/time_series/{station}.nc
  ./data/IA/NeuralHydrology_satellite_10D/attributes/attributes.csv
═══════════════════════════════════════════════════════════════════════════
"""
...
OUTPUT_DIR    = './data/IA/NeuralHydrology_satellite_10D/'
BASINS_DIR    = './AI/LSTM/NeuralHydro_satellite_10D/'
STATIONS_FILE = './Exploring_data/Stations_10j/stations_10j.txt'
...
EXCLUDE_STATIONS = {
    '0000000005728', '0000000005731', '0000000005734',
    '0000000006323', '0000000006343', '0000000006350',
    '0000000006351', '0000000008745', '0000000008749',
    '0000000008750', '0000000008760', '0000000010841',
}
...
# ═══════════════════════════════════════════════════════════════
print("=" * 70)
print("CRÉATION DATASET SATELLITE 10D v2 (+ SNOW + DOY)")
print("=" * 70)

with open(STATIONS_FILE) as f:
    stations_10j = [line.strip() for line in f if line.strip()]
stations_10j = [s for s in stations_10j if s not in EXCLUDE_STATIONS]
print(f"\n  {len(stations_10j)} stations 10j")

grid_10D = pd.date_range(DATE_DEB, DATE_FIN, freq='10D')
print(f"  Grille 10D : {len(grid_10D)} pas")
...
if not has_snow:
    print("\n  ❌ Table era5_snow_bv_jour absente ! Lance d'abord extract_snow_bv_satellite.py")
    conn.close()
    exit(1)
...
for i, code_sta in enumerate(stations_10j):

    # ── 1. ERA5 quotidien (temp, precip, pet) ──────────────────────────
    df_era5 = pd.read_sql('''
    ...
    # ── 5. Construire le DataFrame sur grille 10D ──────────────────────
    df_grid = pd.DataFrame(index=grid_10D)
    df_grid.index.name = 'date'

    # ERA5 J0
    df_grid['precipitation_J0'] = df_era5.reindex(grid_10D)['precip_sum_bv'].values
    df_grid['temperature_J0']   = df_era5.reindex(grid_10D)['temp_moy_bv'].values
    df_grid['pet_J0']           = df_era5.reindex(grid_10D)['pet_sum_bv'].values

    # Snow J0
    if has_snow_sta:
        snow_reindexed = df_snow.reindex(grid_10D)
    ...
    # ERA5 moyennes J3 et J10 depuis le quotidien
    precip_J3  = np.full(len(grid_10D), np.nan)
    temp_J3    = np.full(len(grid_10D), np.nan)
    pet_J3     = np.full(len(grid_10D), np.nan)
    precip_J10 = np.full(len(grid_10D), np.nan)
    temp_J10   = np.full(len(grid_10D), np.nan)

    for j, dt in enumerate(grid_10D):
        # J3
    ...
    water_level = np.full(len(grid_10D), np.nan)
    for j, dt_grid in enumerate(grid_10D):
    ...
    clim_m, clim_s = compute_clim_fenetre(
        grid_10D.to_pydatetime(), water_level, window=CLIM_WINDOW
    )
    df_grid['clim_mean_20j'] = clim_m
    df_grid['clim_std_20j']  = clim_s

    # ── 8. DOY sin/cos ─────────────────────────────────────────────────
    doy = np.clip(grid_10D.dayofyear, 1, 365)
    ...
    ds = xr.Dataset(data_vars, coords={"date": grid_10D.values})
    ds.to_netcdf(nc_path, engine="scipy", format="NETCDF3_CLASSIC")
    stations_exportees.append(code_sta)

    pct_valid = n_valid / len(grid_10D) * 100
    print(f"  [{i+1:2d}/{len(stations_10j)}] {code_sta} — "
...
# FICHIER STATIONS
# ═══════════════════════════════════════════════════════════════
basin_file = os.path.join(BASINS_DIR, 'stations_10j.txt')
...
{'='*60}
  ...
  Grille     : {len(grid_10D)} pas × 10D

  📋 dynamic_inputs pour le YAML :
     - precipitation_J0
     - temperature_J0
     - pet_J0
     - precip_mean_J3
