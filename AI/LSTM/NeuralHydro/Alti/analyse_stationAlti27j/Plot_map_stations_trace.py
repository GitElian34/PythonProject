...
def find_sigma0_file(sta):
    for candidate in [sta, sta.lstrip('0') or '0', str(int(sta.lstrip('0') or 0))]:
...
def couleur_sigma0(s0):
    if s0 >= 35:  return '#2E7D32'   # vert — bon signal
    if s0 >= 30:  return '#F9A825'   # jaune — moyen
    return '#C62828'                  # rouge — mauvais signal
    if np.isnan(nse): return 'gray'
    if nse >= 0.7:    return 'darkgreen'
    if nse >= 0.5:    return 'green'
    if nse >= 0.0:    return 'orange'
...
for sta in stations:
    ...
    if sigma0_path is not None:
        ...
        df_wsh = df_wsh.dropna(subset=['WSH', 'LATITUDE', 'LONGITUDE'])
        fg_wsh = folium.FeatureGroup(name='Points WSH CNES', show=True)
        for _, wrow in df_wsh.iterrows():
            folium.CircleMarker(
                location=[wrow['LATITUDE'], wrow['LONGITUDE']],
                radius=4,
                color='white', weight=1,
                fill=True,
                fill_color=couleur_sigma0(wrow['sigma0']),
                fill_opacity=0.8,
                popup=folium.Popup(
                    f"<b>WSH CNES</b><br>"
                    f"Date : {str(wrow['date'])[:10]}<br>"
                    f"WSH : {wrow['WSH']:.3f} m<br>"
                    f"Sigma0 : {wrow['sigma0']:.1f} dB<br>"
                    f"Cycle : {int(wrow['CYCLE_NUMBER']) if not pd.isna(wrow['CYCLE_NUMBER']) else 'N/A'}",
                    max_width=200),
                tooltip=f"WSH={wrow['WSH']:.2f}m σ0={wrow['sigma0']:.1f}dB",
            ).add_to(fg_wsh)
        fg_wsh.add_to(m)
