"""
create_dataset_feat10j.py — VERSION 4
═══════════════════════════════════════════════════════════════════════════
Dataset modèles 2 et 3 — Features agrégées avec NEIGE + CLIMATOLOGIE + DOY
  - 1 point tous les 10j (décalages 0 à 9)
  - Variables dynamiques à chaque point :
      precipitation_J0, temperature_J0, pet_J0          (ERA5 du jour J)
      snow_depth_J0, snowmelt_J0                        (ERA5-Land du jour J)
      precip_mean_J3, temp_mean_J3, pet_mean_J3         (moyennes 3j)
      snow_depth_mean_J3, snowmelt_mean_J3
      precip_mean_J10, temp_mean_J10, pet_mean_J10      (moyennes 10j)
      snow_depth_mean_J10, snowmelt_mean_J10
      clim_mean, clim_std                               (climatologie WL normalisée par DOY)
      clim_mean_20j, clim_std_20j                       (clim fenêtrée ±20j, excluant l'année courante) [NEW]
      doy_sin, doy_cos                                  (encodage cyclique du jour de l'année)
      water_level                                       (z-score normalisé)

  - 10 fichiers .nc par station : {code_sta}_d0.nc ... {code_sta}_d9.nc
  - Dossier : ./data/IA/NeuralHydrology_feat10j/

  - FILTRE : exclut les stations flaggées (flag_capteur IS NOT NULL)
═══════════════════════════════════════════════════════════════════════════
"""
...
N_STATIONS    = 2000  # mets une grande valeur pour prendre tout
VAL_RATIO     = 0.2
SEED          = 42
MIN_GAP_JOURS = 60
MIN_DIST_M    = 500
FREQ_JOURS    = 10
RESET         = True   # ← True pour tout regénérer (efface time_series)
CLIM_WINDOW   = 20     # ← Demi-fenêtre en jours pour la clim fenêtrée (±20j)
...
# Dict {code_sta: DataFrame(doy, wl_mean, wl_std)} pour lookup rapide
clim_dict = {}
...
def compute_clim_fenetre(wl_series, window=CLIM_WINDOW):
    """
    Pour chaque date de wl_series, calcule la moyenne et le std du water_level
    normalisé sur une fenêtre ±window jours autour du DOY, en excluant l'année
    courante (leave-one-year-out).

    Args:
        wl_series: pd.Series indexée par DatetimeIndex, water_level déjà normalisé (z-score)
        window: demi-fenêtre en jours (default 20 → fenêtre totale de 41j)

    Returns:
        clim_mean_20j: np.array (même longueur que wl_series.index complet)
        clim_std_20j:  np.array
    """
    # On travaille uniquement sur les valeurs non-NaN pour construire la clim
    wl_valid = wl_series.dropna()

    if len(wl_valid) < 30:
        # Pas assez de données → prior neutre
        n = len(wl_series)
        return np.zeros(n, dtype=np.float32), np.ones(n, dtype=np.float32)

    # Pré-calculer DOY et année pour les valeurs valides
    valid_doys  = np.clip(wl_valid.index.dayofyear, 1, 365)
    valid_years = wl_valid.index.year
    valid_vals  = wl_valid.values

    # DOY et année pour TOUTES les dates (y compris NaN water_level)
    ...
    for i in range(len(all_dates)):
        ...
        # Distance circulaire en jours (gère le passage déc → jan)
        doy_diff = np.abs(valid_doys - cur_doy)
        doy_diff = np.minimum(doy_diff, 365 - doy_diff)

        # Masque : dans la fenêtre ET pas la même année
        ...
        if len(vals_in_window) >= 3:
            ...
            if clim_std[i] < 0.01:
                clim_std[i] = 1.0  # éviter std ~0
        else:
            # Pas assez de points → prior neutre
            clim_mean[i] = 0.0
            clim_std[i]  = 1.0
...
# ═══════════════════════════════════════════════════════════════
# EXPORT .NC — features agrégées + neige + clim + clim_20j + doy
# ═══════════════════════════════════════════════════════════════
...
for i, (_, row) in enumerate(candidats.iterrows()):
    ...
    # Charger ERA5 quotidien + neige + water_level via JOIN sur la date
    df = pd.read_sql('''
        SELECT
    ...
    # ── Remplir les NaN neige par 0 (zone sans neige = manquant = 0) ────────
    ...
    # sin/cos DOY (toujours disponible, pas de NaN)
    df['doy_sin'] = np.sin(2 * np.pi * doy / 365)
    df['doy_cos'] = np.cos(2 * np.pi * doy / 365)

    # Climatologie par DOY (mean + std du water_level normalisé)
    if code_sta in clim_dict:
        ...
        # Station sans climatologie → on met 0 / 1 (prior neutre)
        df['clim_mean'] = 0.0
        df['clim_std']  = 1.0
...
# ═══════════════════════════════════════════════════════════════
# ATTRIBUTES — incluant elevation/slope/strahler
# ═══════════════════════════════════════════════════════════════
...
📋 Variables dynamiques pour le yaml :
   ...
   static_attributes:
     ...   (avec elevation_mean et slope_mean)
""")
