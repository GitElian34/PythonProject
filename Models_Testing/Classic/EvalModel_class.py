"""
eval_zeroshot_hwnext_10j.py
════════════════════════════════════════════════════════════════════════
Évaluation zero-shot du modèle AR-LSTM 10j (arlstm_feat10jLow_modele2,
entraîné sur train_basins de NeuralHydro_feat10j_low) sur les nouvelles
données HydroWeb Next 10j.

Vérification préalable faite (via comm -12 sur les listes de stations) :
les 63 stations de NeuralHydrology_hydroweb_next/10j sont entièrement
disjointes de AI/LSTM/NeuralHydro_feat10j_low/train_basins.txt du modèle
→ évaluation zero-shot valide, aucune fuite de données.
...
EPOCH  = 7  # ⚠️ à confirmer : vérifier quelles époques existent réellement
            #    dans runs/<MODEL>/ avant de lancer

PERIOD = "validation"

RUN_DIR       = Path(f"./runs/{MODEL}")
STATIONS_FILE = Path("./toutes_stations_hwnext_10j.txt")
DATA_DIR      = Path("./data/IA/NeuralHydrology_hydroweb_next/10j")
OUT_CSV       = Path("./Models_Testing/Residus/residuals_10j_hwnext.csv")
...
# ═══════════════════════════════════════════════════════════════
print("=" * 60)
print(f"ÉVALUATION ZERO-SHOT HYDROWEB NEXT 10j")
...
# Score outlier (même formule que les autres évaluations)
